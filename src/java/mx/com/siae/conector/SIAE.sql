...
CREATE TABLE IF NOT EXISTS `SIAE`.`Responsables` (
  ...
  idUsuario VARCHAR(20) NOT NULL,
  PRIMARY KEY (idUsuario),
  CONSTRAINT `fk_Responsables_Usuarios1` FOREIGN KEY (idUsuario) REFERENCES Usuarios (idUsuario)
);
...
CREATE TABLE IF NOT EXISTS `SIAE`.`Cursos` (
  ...
  idAsignatura INT NOT NULL,
  idResponsable VARCHAR(20) NOT NULL,
  PRIMARY KEY (idCurso), CONSTRAINT `fk_Curso_Asignaturas` FOREIGN KEY (idAsignatura) REFERENCES Asignaturas (idAsignatura),
  CONSTRAINT `fk_Curso_Responsable1` FOREIGN KEY (idResponsable) REFERENCES Responsables (idUsuario)
);
-- Table `SIAE`.`Alumnos`
...
CREATE TABLE IF NOT EXISTS `SIAE`.`Dependencias` (
  ...
  CONSTRAINT `fk_Asignaturas_has_Asignaturas_Asignaturas1` FOREIGN KEY (idAsignatura_p) REFERENCES Asignaturas (idAsignatura),
...
CREATE TABLE IF NOT EXISTS `SIAE`.`Sesiones` (
  ...
  CONSTRAINT `fk_Sesiones_Curso1` FOREIGN KEY (idCurso) REFERENCES Cursos (idCurso),
  CONSTRAINT `chk_Sesiones_horario1` CHECK( hora_inicio < hora_fin ) ENFORCED
);
...
BEGIN
    ...
    START TRANSACTION;
        INSERT INTO Usuarios VALUES(in_idUsuario, in_nombre_1, in_nombre_2, in_nombre_3, in_apellido_pat, in_apellido_mat, in_correo_inst, in_rol, SHA2(in_contra, 512), in_num_tel);
    IF t_error THEN ROLLBACK; SELECT t_msg FROM dual; ELSE COMMIT; END IF;
END $$
DELIMITER ;
;
...
CALL proce_nuevo_user('18011225','Nazareth','Zurisay',null,'Morales','Gino','nzmorales@itsoeh.edu.mx','A','nzmorales','7731103479');
...
DELIMITER ;
;
-- area IN('CB', 'CI', 'IA', 'DI', 'CS', 'CE','CC')  estado in('E', 'D')  credito BETWEEN 0 AND 10
...
) COMMENT 'Este procedimiento creara una nueva area de apoyo, validando el horario asignado al representante' DETERMINISTIC
BEGIN
    DECLARE t_error BOOLEAN;
...
CALL proce_nueva_sesion(1001, 'Martes',    '09:00','11:00');
CALL proce_nueva_sesion(1005, 'Miercoles', '07:00','09:00');
CALL proce_nueva_sesion(1006, 'Lunes',     '14:00','16:00');
CALL proce_nueva_sesion(1007, 'Viernes',   '16:00','17:00');
CALL proce_nueva_sesion(1008, 'Jueves',    '09:00','11:00');
CALL proce_nueva_sesion(1010, 'Martes',    '11:00','13:00');
CALL proce_nueva_sesion(1001, 'Miercoles', '09:00','11:00');
CALL proce_nueva_sesion(1005, 'Martes',    '07:00','09:00');
CALL proce_nueva_sesion(1006, 'Jueves',    '14:00','16:00');
CALL proce_nueva_sesion(1007, 'Martes',    '16:00','17:00');
CALL proce_nueva_sesion(1008, 'Lunes',     '09:00','11:00');
CALL proce_nueva_sesion(1010, 'Viernes',   '11:00','13:00');
CALL proce_nueva_sesion(1003, 'Lunes',     '10:00','12:00');
CALL proce_nueva_sesion(1025, 'Viernes',   '07:00','09:00');
...
CALL proce_nueva_sesion(1025, 'Lunes',     '07:00','09:00');
...
CALL proce_nueva_sesion(1014, 'Jueves',    '09:00','12:00');
CALL proce_nueva_sesion(1016, 'Lunes',     '12:00','13:00');
CALL proce_nueva_sesion(1017, 'Viernes',   '16:00','17:00');
CALL proce_nueva_sesion(1011, 'Martes',    '09:00','11:00');
CALL proce_nueva_sesion(1012, 'Viernes',   '11:00','12:00');
CALL proce_nueva_sesion(1015, 'Viernes',   '07:00','09:00');
CALL proce_nueva_sesion(1041, 'Martes',    '11:00','13:00');
CALL proce_nueva_sesion(1045, 'Viernes',   '07:00','09:00');
...
BEGIN
    ...
    DECLARE CONTINUE HANDLER FOR t_msg_error_fk BEGIN SET t_msg = '1452 foreign key constraint'; SET t_error = true; END;
    DECLARE CONTINUE HANDLER FOR t_msg_error_null BEGIN SET t_msg = '1048 Can not NULL'; SET t_error = true; END;
    SELECT in_fecha_ingreso < CURRENT_DATE INTO t_error FROM DUAL;
...
DELIMITER ;
;
-- SELECT '2021-04-22' < CURRENT_DATE FROM DUAL;
...
CREATE FUNCTION funci_nombre_user ( in_idUsuario VARCHAR(20) ) RETURNS VARCHAR(225)
    COMMENT 'Esta fución obtendra el nombre completo del usuario' DETERMINISTIC
BEGIN
    DECLARE nombre VARCHAR(225) DEFAULT '';
    SELECT concat(nombre_1 , ' ', COALESCE(nombre_2, ''), ' ', COALESCE(nombre_3, ''), 
        apellido_pat, ' ', COALESCE(apellido_mat, '')) INTO nombre
...
BEGIN
    ...
    IF ob_contra = ob_in_contra THEN SET ob_verif = TRUE; SET act = 1; ELSE SELECT act, sem FROM DUAL; END IF;
    IF ob_verif = TRUE THEN
        ...
        ELSE
            ...
            IF ob_tipoR = 'D' THEN SET sem = -2; END IF;
        END IF;
...
DELIMITER ;
;
-- CALL proce_iniciar_sesion('18011126', 'dhernandezr'); CALL proce_iniciar_sesion('D00001', 'caguilar');
###########################################################################################
DROP PROCEDURE IF EXISTS proce_reporte_cursos;
DELIMITER $$
CREATE PROCEDURE proce_reporte_cursos() COMMENT 'Este prosedimiento obtiene los datos de los cursos disponibles' DETERMINISTIC
BEGIN
    ...
    ON(cas.idR=u.idR) ORDER BY cas.semestre ASC;
END $$
DELIMITER ;
;
...
BEGIN
    ...
    FROM
    (   SELECT idAA, url, dia, horario, funci_nombre_user(idUsuario) AS docente, idA, codigo 
        FROM
...
CREATE PROCEDURE proce_cambio_status_alumno( in in_idCurso INT, in in_matricula VARCHAR(20), in in_reporte CHAR(1) )
    COMMENT 'Este procedimiento realiza el reporte de su estado respecto a un curso, (P) pendiente, (R) reprovado, (A) aprovado' DETERMINISTIC
BEGIN
    DECLARE t_error BOOLEAN;
    DECLARE t_msg VARCHAR(100) DEFAULT 'OK';
    DECLARE t_msg_error CONDITION FOR 1062;
...
BEGIN
    ...
    FROM
    (
        SELECT c.idC, a.idA, c.idR, tipo, estado, cupo, nombre 
        FROM
...
CREATE PROCEDURE proce_consulta_asesoria()
    DETERMINISTIC
BEGIN
    SELECT rua.idAA, asignatura, url, dia, horario, estado, docente, extra
    FROM
        ...
        FROM
            (
                ...
                FROM AreasApoyo WHERE idAsignatura is not null
            ) a
        JOIN
            (   SELECT r.idR, docente FROM
                    (SELECT idUsuario AS idR FROM Responsables WHERE tipo = 'D') r
...
DROP PROCEDURE IF EXISTS proce_estado_asesoria;
DELIMITER $$
CREATE PROCEDURE proce_estado_asesoria( in in_id INT, in in_estado CHAR(1) ) COMMENT 'Este procedimiento cambia el estado de una asesosia, comando en cuenta que no entre en conflicto de horarios, con las asesorias habilitadas' DETERMINISTIC
BEGIN
    DECLARE count INT DEFAULT -1;
    ...
    ELSE UPDATE AreasApoyo SET estado = in_estado WHERE (idAreasApoyo = in_id); END IF;
END $$
DELIMITER ;
;
...
DELIMITER $$
CREATE PROCEDURE proce_eliminar_sesion( in in_idSesion INT) COMMENT 'Este procedimiento elimina una sesión' DETERMINISTIC
BEGIN
...
DELIMITER $$
CREATE PROCEDURE proce_update_usuario( in in_id VARCHAR(20), in in_foto MEDIUMBLOB) COMMENT 'Este procedimiento actualiza los datos del usuario' DETERMINISTIC
BEGIN
...
INSERT INTO Cursos_Alumnos VALUES
       ...
       (1015,'R','R','18011530'),
       (1016,'S','P','18011530');
--    curso  state repo   matricula
--  1 '1001', 'S', 'P', '18011530' 
--  2 '1003', 'A', 'A', '18011530' 
--  3 '1005', 'R', 'P', '18011530' * 1
--  4 '1006', 'A', 'R', '18011530' * 2
--  5 '1007', 'R', 'R', '18011530' * 3
--  6 '1008', 'A', 'R', '18011530' * 4
--  7 '1010', 'R', 'R', '18011530' * 5
--  8 '1011', 'A', 'R', '18011530' * 6
--  9 '1012', 'A', 'R', '18011530' * 7
-- 10 '1014', 'A', 'R', '18011530' * 8
-- 11 '1015', 'R', 'R', '18011530' * 9
-- 12 '1016', 'S', 'P', '18011530' 
-- 13 '1017', 'S', 'P', '18011530' 

SELECT * FROM Cursos;
SELECT * FROM Cursos_Alumnos WHERE matricula = '18011530';

SELECT * FROM Cursos_Alumnos WHERE matricula = '18011530' AND ((estado = 'A' AND reporte = 'R') OR (estado = 'R' AND (reporte = 'R' OR reporte = 'P')));
-- MAterias aprobadas 
-- Semestre: round( (datediff( CURRENT_DATE, ob_ingreso)/183.5) + 1 )
SELECT * FROM Cursos_Alumnos WHERE matricula = '18011530' AND (estado = 'A' AND reporte = 'A');
SELECT round( (datediff( CURRENT_DATE, fecha_ingreso)/183.5) + 1 ) FROM Alumnos WHERE matricula = '18011530';

SELECT idAsignatura_p AS padre, idAsignatura_h AS hija, semestre, nombre, credito
  FROM Dependencias
  JOIN Asignaturas
    ON (idAsignatura_h = idAsignatura);

SELECT count(*)
FRAlumnosOM
( SELECT idCurso FROM (SELECT idAsignatura_h AS hija FROM Dependencias WHERE idAsignatura_p = 101) p JOIN Cursos c ON(p.hija=c.idAsignatura) ) tmp
JOIN
(SELECT idCurso FROM Cursos_Alumnos WHERE matricula = '18011530' AND (estado = 'A' ANd reporte = 'A')) alu
ON(tmp.idCurso=alu.idCurso);


SELECT idCurso FROM Cursos_Alumnos WHERE matricula = '18011530' AND (estado = 'A' ANd reporte = 'A');
SELECT * FROM Cursos WHERE idAsignatura = 101;